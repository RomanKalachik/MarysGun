...
// ShutterButton()
// Inputs:
//   durationInTenthsSec  length of time to hold shutter button down
//   timeOutInSecs        timeout in secs to wait for picture to be
//                        taken and downloaded (max 60 secs)
//   pszFilename          optional string in which to store the name of the
//                        saved image. Set to NULL if not required
...
//
// Description:
//   Take a picture by simulating the pressing of the shutter release
//   button on the camera and optionally wait for it to be saved to disk.
//   Requires DSLR Remote Pro for Windows v2.3.3 or later
//
//-----------------------------------------------------------------------
DSLRREMOTELIB_API int __stdcall ShutterButton(
							int   durationInTenthsSec,
							int   timeoutInSecs,
							char* pszFilename,
							int   numChars
							);
...
// SaveEvfFrame()
// Inputs:
//  path       pathname to use when saving the EVF frame
//	Note: EVF must be displayed before calling this function
//
// Returns:
//   0 - Success
...
//
//-----------------------------------------------------------------------
DSLRREMOTELIB_API int __stdcall SaveEvfFrame(const char* path);
